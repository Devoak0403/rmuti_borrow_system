<?php 
		...
		$j(document).ready(function(){
			...
			function add_input_field(count)
			{
				...
				html += '<td><select name="item_name[]" class="form-control selectpicker" data-live-search="true"><option value="">เลือกครุภัณฑ์</option><?php echo fill_item_select_box($connect); ?></select></td>';
                ...
                // html += '<td><input type="number" name="item_quantityday[]" class="form-control item_quantity" /><small id="emailHelp" class="form-text text-muted">ใส่เป็นเลข เช่น 5 , 10 , 20 , 30</small></td>';
			...
			$j('#insert_form').on('submit', function(event){
				...
				// $j('.item_quantityday').each(function(){
		
				// 	if($j(this).val() == '')
				// 	{
		
				// 		error += "<li>Enter Item QuantityDay at "+count+" Row</li>";
