...
if(isset($_POST['submit'])){
    ...
    if($h_type ==1 ){
        ...
        $stmt2=$conn->prepare("INSERT INTO history(u_name,u_id,h_type,h_value,lent_start,lent_end,status_id,item_id,u_code)VALUES(?,?,?,?,?,?,?,?,?)");
        ...
        $stmt2->bindParam(6, $empty_lent);

        $stmt2->bindParam(7, $status_id);

        $stmt2->bindParam(8, $item_id);

        $stmt2->bindParam(9, $u_code);
        ...
        $result3 = $stmt3->execute();
            ...
            if($result2 && $result3){
                ...
                $sMessage = "มีรายการเบิกอุปกรณ์ " . " \n";
                $sMessage .= "ผู้ทำรายการ : " . $fname.' '.$lname . " \n";
        ...
        if($item_old < $h_value){
                ...
                header("refresh:4; url=../form_lent_user.php");

        }else{



            $stmt4=$conn->prepare("INSERT INTO history(u_name,u_id,h_type,h_value,lent_start,lent_end,item_id,u_code,status_id)VALUES(?,?,?,?,?,?,?,?,?)");
            ...
            $stmt4->bindParam(7, $item_id);

            $stmt4->bindParam(8, $u_code);

            $stmt4->bindParam(9, $status_id);
            ...
            if ($result4 && $result5) {
                ...
                header("refresh:4; url=../index_user.php");
                ...
                header("refresh:2; url=../form_lent_user.php");
